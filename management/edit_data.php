...
<body>
    <div>
        <div style="display: inline-block; margin: 1em; vertical-align : top ">
            <p>輸入想要新增的row的資料</p>
            <form action='add_data_check.php' method='get'>
                <p>
                <p>Run<br><input type='text' placeholder='ex: SRR1153450' name='add_input_run'></p>
        ...
        <div style="display: inline-block; margin: 1em; vertical-align : top ">
            <p>輸入想要編輯的row的Run</p>
            <form action='edit_data_last.php' method='get'>
                <p>
                <p><?php echo $COLUMN[0]; ?><br><input type='text' value='<?php echo $edit_input_run; ?>' readonly="readonly" name='edit_input_<?php echo $COLUMN[0]; ?>'></p>
                <?php

                $sql = "SELECT * FROM `SraBioSampleInfo` WHERE `Run`='$edit_input_run'";
                $column_value = mysqli_query($link, $sql) or die("fail sent");
                $temp = mysqli_fetch_array($column_value);
                ...
                for ($i = 1; $i < $column_num; $i++) {
                    ...
                    if ($COLUMN[$i] == 'geographic_location') {
                        ...
                        if ($temp[$COLUMN[$i]] != null) {
                            $nation_and_state = explode(':', $temp[$COLUMN[$i]]);
                            echo "original nation: ".$nation_and_state[0]."<br>";
                            echo "請重新選擇nation(不管有沒有改)<br>";

                            echo "<select style='width:170px;' name='edit_input_" . str_replace(' ', '', $COLUMN[$i]) . "_nation' >";
                            ...
                            echo "</select>";
                            echo "<br><input type='text' style='width:170px;' name='edit_input_" . str_replace(' ', '', $COLUMN[$i]) . "_state' placeholder='state' value=" . $nation_and_state[1] . ">";
                            echo "</p>";
                        } else {
                            echo "original nation: null<br>";
                            echo "請選擇nation<br>";
                            echo "<select style='width:170px;' name='edit_input_" . str_replace(' ', '', $COLUMN[$i]) . "_nation'>";
                            ...
                            echo "</select>";
                            echo "<br><input type='text' style='width:170px;' name='edit_input_" . str_replace(' ', '', $COLUMN[$i]) . "_state' placeholder='state'>";
                            echo "</p>";
                        }
                    } else {
                        echo "<p>" . $COLUMN[$i] . "<br><input type='text' name='edit_input_" . str_replace(' ', '', $COLUMN[$i]) . "' value=" . $temp[$COLUMN[$i]] . "></p>";
                    }
                }
                ?>
                <input type='submit' value='確定'>
