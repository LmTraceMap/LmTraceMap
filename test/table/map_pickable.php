...
for ($i = 0; $i < 50; $i++) {
  ...
  $germ_nation = explode(":", $germ_location[0]);
  if ($germ_nation[0] == 'USA') {
    $germ_nation[0] = 'United States';
  } else if ($germ_nation[0] == 'South Korea') {
    $germ_nation[0] = 'Korea (South)';
  } else if ($germ_nation[0] == 'Korea') {
    $germ_nation[0] = 'Korea (South)';
  } else if ($germ_nation[0] == 'Russian') {
    $germ_nation[0] = 'Russian Federation';
  }
  ...
  echo "<br>";
  */
  array_push($rank[$i], $germ_nation[0]);  #$rank[x][0]->細菌名字, $rank[x][1]距離, $rank[x][2]->該細菌的國家
}

$geo = array(); //總共有幾種國家 $geo[x][0]->地名,$geo[x][1]->該地名的數量(該地名有幾個菌),$geo[x][2]->該地名的lat,$geo[x][3]->該地名的lng
//$Yes_No = array(); //存國家經緯度有沒有在json裡(1有0無)
$index_count = 0;
//$userYN = 0;
...
for ($i = 0; $i < 50; $i++) {
  ...
  if ($state == 0 && $rank[$i][2] != null) //如果該地名沒在$geo裡面
  {
    ...
    $temp = $geo[$index_count][0];
    $sql = "SELECT `latitude`,`longtitude` FROM `nation_located` WHERE `nation`='$temp'";
    $result = mysqli_query($link, $sql) or die($temp);
    $nation = mysqli_fetch_array($result);
    $column_num = mysqli_num_rows($result); //有幾個column

    if ($column_num != 0) {  //如果資座標有該國家的對應座標
      $geo[$index_count][2] = $nation['latitude'];
      $geo[$index_count][3] = $nation['longtitude'];
    } else {  //如果資料庫沒有該國家的對應座標
      $geo[$index_count][2] = 0;
      $geo[$index_count][3] = 0;
    }
...
/*
// 從檔案中讀取資料到PHP變數 
$json_string = file_get_contents('../try.json');
// 用引數true把JSON字串強制轉成PHP陣列 
$data = json_decode($json_string, true);

for ($i = 0; $i < $index_count; $i++) {
  $geoname = $geo[$index_count][0];
  if ($data[$geoname]["lat"]) {
    $Yes_No[$index_count] = 1;
  } else
    $Yes_No[$index_count] = 0;
}

if ($data[$user_country]["lat"]) {
  $userYN = 1;
}
*/
...
<body>
  <div style="position: relative; height: 15%; width: 100%;background-color: #f1f1dd;">
    <div id="input_name_p"><a href="../upload.php" style="color:black; text-decoration:none;">
        <div>LmTraceMap</div>
        <div style="font-size: 15pt;">(L.monocytogenes global tracking web tool)</div>
      </a></div>
    <div id="sample"><a href="../sample.html" style="color:black;">Input/Output<br>Sample</a></div>
    <div id="help"><a href="../help.html" style="color:black;">Help</a></div>
  </div>

  <div id="upload_background">
    <div style="background:white;position:relative;width:5%;height:100%;float:left;">
      <div class="border" id="change_map" onclick="ChangeToMap()" style="background:#6E6EFF;position:relative;width:90%;height:90px;color:white;margin-left:10%;text-overflow:ellipsis;word-break:break-all;font-size:120%;text-align:center;top:0.5%;padding-left:12%;padding-right:80%;">M<br>A<br>P</div>
      <div class="border" id="change_form" onclick="ChangeToForm()" style="background:#FF6E6E;position:relative;width:60%;height:120px;color:white;margin-left:40%;text-overflow:ellipsis;word-break:break-all;font-size:120%;text-align:center;top:0.5%;padding-left:12%;padding-right:20%;">F<br>O<br>R<br>M</div>
    </div>
    <div id="mapid" style="height: 100%;width:90%;position:absolute;border-style:groove;margin-left:5%;z-index:2;">
      ...
      <script type="text/javascript">
        ...
        //Yes_No = <?php /*echo json_encode($Yes_No);*/ ?>;
        ...
        function check() {
          for (var i = 0; i < index_count; i++) {
            codeAddress(germ_geo[i][0], germ_geo[i][1], germ_geo[i][2], germ_geo[i][3]);
          }
          /*
          if (nextAddress < index_count) {
            setTimeout("codeAddress('" + germ_geo[nextAddress][0] + "', '" + germ_geo[nextAddress][1] + ", '" + germ_geo[nextAddress][2] + ", '" + germ_geo[nextAddress][3] + "')", delay);
            nextAddress++;
          }
          */
        }
        ...
        function user_location() {
          <?php
          $sql = "SELECT `latitude`,`longtitude` FROM `nation_located` WHERE `nation`='$user_country'";
          $result = mysqli_query($link, $sql) or die("fail sent 3");
          $nation = mysqli_fetch_array($result);
          $user_country_lat = $nation['latitude'];
          $user_country_lng = $nation['longtitude'];
          ?>
          codeAddress('<?php echo $user_country; ?>', -1, '<?php echo $user_country_lat; ?>', '<?php echo $user_country_lng; ?>'); //使用者的位置
          check();
        }

        function codeAddress(geo, num, lat, lng) {
          console.log(geo);
          var address = geo;
          //var lat;
          //var lng;
          /*
                    if (userYN == 0 || Yes_No[num_array] == 0) {
                      $.ajax({
                      ...
                      $.ajax({
                        ...
                        success: function(data) {
                          var des = data.search('>')
                          var des2 = data.indexOf('\n', des);
                          var latstr = data.substring(des + 2, des2);
                          lat = parseFloat(latstr, 10);
                          des = data.indexOf('>', des2);
                          des2 = data.indexOf('\n', des);
                          var lngstr = data.substring(des + 2, des2);
                          ...
                          popsign = "<p style='text-align: center;font-size:18px;margin:0px;background:gainsboro;border-top-left-radius:4px;border-top-right-radius:4px;'>" + geo+lat+lng + "</p>";
          ...
          */
          ...
          popsign = "<p style='text-align: center;font-size:18px;margin:0px;border-top-left-radius:4px;border-top-right-radius:4px;'>" + geo + "</p>";

          if (num == -1) {
            popsign += "<p style='text-align: center;'>germ loaction uploaded by user</p>"
          } else {
            count = 0;
            for (var i = 0; i < 50; i++) {
              if (geo == germ[i][2]) {
        ...
        function pop(seq, rank, germ_name, distance) {
          if (seq == 1)
            popsign = popsign + "<table style='text-align: center;width:100%;'><tr><td style='border: 1px solid #333;'>seq</td><td style='border: 1px solid #333;'>rank</td><td style='border: 1px solid #333;'>gene name</td><td style='border: 1px solid #333;'>distance</td></tr>";
    ...
    <div id="table_c" style="height: 100%;width:90%;position:absolute;border-style:groove;margin-left:5%;z-index:1;background:white; ">
      <div id="table">
        <div style="position:relative;height:7%;">
          <div style="position:fixed; display: block; z-index: 100;">
            <!--下載table的按鈕-->
            <a id="exportcsv" class="btn btn-success" style="color:white; cursor:pointer;" download="前50近似的菌info.csv">download .csv</a>
            <a id="exportxls" class="btn btn-success" style="color:white; cursor:pointer;" download="前50近似的菌info.xls">download .xls</a>
            <input type="button" id="selectInput" value="choose column" class="btn btn-success" style="color:white; cursor:pointer;">
            <ul id="optionLis" style="position: absolute; margin-left:195px;width:250px;height:300px;padding-left:0px;background:white;overflow-y:scroll;overflow-x:hidden;border:solid 1px black;">
              <?php
              for ($i = 0; $i < $column_num; $i++) {
        ...
        <div style="position:relative;height:92%; width:100%;overflow:scroll;top:1%">
          <div style="position:absolute;">
            <table class=" sort_table tree table table-condensed table-hover table-bordered" id="treegridTable" style="position: relative ;margin:0px;">
              <thead>
  ...
  <footer style="position:absolute;bottom: 0%; width: 100%; height: 20%; background-color: #c5c09a; overflow-y: scroll;">
    <div style="position:relative; top:5%">
      <div style="position:absolute; display: block-inline; left:15px; width: 30%;">
        <p>traffic:
          <img style="width: 15%;" src="https://hitwebcounter.com/counter/counter.php?page=7844372&style=0007&nbdigits=5&type=page&initCount=0" title="Free Counter" Alt="web counter" /></a>
        </p>
      </div>
  ...
  <script>
    window.onload = function() {
      ...
      table_column_display.innerHTML = "<th style='width: 200px; font-size:12pt;'>rank</th>";
      table_column_display.innerHTML += "<th style='width: 200px; font-size:12pt;'>gene name</th>";
      table_column_display.innerHTML += "<th style='width: 200px; font-size:12pt;'>distance</th>";
      table_column_display.innerHTML += "<th style='width: 200px; font-size:12pt;'>geographic_location</th>";
      ...
      for (var a = 1; a <= 50; a++) {
        ...
        display_string += "<td style='font-size:10pt;'>" + a + "</td>"; //排名
        display_string += "<td style='font-size:10pt;'>" + rank50[a - 1][0] + "</td>"; //名字rank[x][0]
        display_string += "<td style='font-size:10pt;'>" + rank50[a - 1][1] + "</td>"; //距離rank[x][1]
        display_string += "<td style='font-size:10pt;'>" + geographic_location[a-1].replace(',', ';') + "</td>"; //距離rank[x][1]
        display_string += "</tr>";
          ...
          if (column_select[i] == this.innerHTML) {
            ...
            for (var a = 1; a <= 50; a++) {
              ...
              for (var j = 0; j < column_select.length; j++) {
                ...
                display_string += "<td style='font-size:10pt;'>" + temp.replace(',', ';') + "</td>";
              }
              display_string += "</tr>";
              table_info_display.innerHTML = display_string;
            }
