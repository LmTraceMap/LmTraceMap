...
die "\nPlease input at least one genome contig files.\n\n" if($contig_num < 1);
...
for(my $i=5;$i<=10;$i++) {
	my $var_th = $i/10;
	system("/usr/bin/python $programPath/varianceThreshold.py $outPath/alleleMatrix $var_th > $outPath/scheme/variance_$i.scheme");
}
