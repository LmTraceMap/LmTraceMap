...
if(length($userScheme) != 0) {
	...
	open(PAN, ">$outPath/panDB_all_tmp.fa") or die("Could not open panDB_all_tmp.fa file.");
	close(PAN);
	system("chmod 0777 $outPath/panDB_all_tmp.fa");
	...
	open(OU, ">$outPath/panDB_all.fa");
	close(OU);
	system("chmod 0777 $outPath/panDB_all.fa");
...
system("/usr/bin/sudo chmod 0777 $outPath/panDB_all.fa");
...
for(my $n=1;$n<=$assemblyNum;$n++) {
	...
	system("cp $outPath/Assembly/A00000001.fa $outPath/AssemblyDB.fa");

	system("/usr/bin/makeblastdb -in $outPath/AssemblyDB.fa -dbtype nucl -out $outPath/AssemblyDB");
	system("/usr/bin/blastn -num_threads $threads -query $outPath/panDB_all.fa -db $outPath/AssemblyDB -out $outPath/profiling.bls -outfmt '6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore'");

system("usr/bin/sudo chmod 0777 $outPath/profiling.bls");
	##### read profiling.bls #####
...
open(OUT, ">$outPath/locus_profile.txt");
...
open(OUT, ">$outPath/allele_profile.txt");
print OUT ("$matrixLabel");
for(my $i=1;$i<=$locusNum;$i++) {
	my $locusID = (sprintf "SAL%07d", $i);
	next if(length($userScheme) != 0 && $userScheme{$locusID} != 1);
